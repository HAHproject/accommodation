...
public class AccoController {
    ...
    public List<Accommodation> getAllByCondition(){
        return accoService.getAllByCondition();
    }

    @GetMapping("/apply/{id}")
    public AccommodationResponse getById(@PathVariable("id") Long id){
        // 이건 어드민이 접근할 수 있어야한다...
        return accoService.getById(id);
    }

    @GetMapping("/me/{id}")
    public AccommodationResponse getByOwnerId(@PathVariable("id") String uid){

        return accoService.getByOwnerId(uid);
    }
